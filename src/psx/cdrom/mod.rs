...
pub struct Cdrom {
    ...
    #[serde(default, alias = "sixstep")]
    sixstep_per_channel: [usize; 2],
    ringbuf: [[i16; 0x20]; 2],
}

impl Cdrom {
    pub fn new(game_filepath: &str) -> Cdrom {
        let disc = match disc::open(game_filepath) {
            Ok(d) => Some(d),
            Err(e) => {
                log_error!("CDROM", "failed to open '{}': {}", game_filepath, e);
                None
            }
        };
    ...
    pub fn load_disc(&mut self, filepath: &str) {
        match disc::open(filepath) {
            Ok(d) => self.disc = Some(d),
            Err(e) => {
                log_error!("CDROM", "failed to load disc '{}': {}", filepath, e);
                self.disc = None;
            }
        }
    }

    fn read_sector_raw(&mut self, lba: u64, out: &mut [u8; 2352]) -> bool {
        match self.disc.as_mut() {
            Some(d) => match d.read_sector(lba, out) {
                Ok(()) => true,
                Err(e) => {
                    log_warn!("CDROM", "read LBA {} failed: {}", lba, e);
    ...
    pub fn get_disc_id(&mut self) -> String {
        ...
        if !self.read_sector_raw(16, &mut sector) {
            log_warn!("CDROM", "PVD read failed, using NODISC id");
            return String::from("NODISC");
        }

        let pvd = &sector[DATA_OFFSET..DATA_OFFSET + 2048];

        if pvd[0] != 0x1 {
            log_warn!("CDROM", "PVD type mismatch (got {:#04x})", pvd[0]);
            return String::from("UNKNOWN");
        }

        if pvd[40] == 0x20 && pvd[41] == 0x20 {
            return String::from("UNKNOWN");
        }

        let mut id: String = String::from_utf8_lossy(&pvd[40..72]).to_string();
        id.retain(|c| !c.is_whitespace());
        if id.is_empty() { id = "UNKNOWN".to_string(); }
        id
    }
    ...
    fn tick_drive(&mut self, spu: &mut Spu, clocks: usize) {
        ...
        match self.drive_mode {
            ...
            CdromDriveMode::Seek => {
                ...
                self.last_subq.ss = self.seek_second.saturating_sub(2);
            ...
            CdromDriveMode::Play => {
                ...
                let lba = self.get_seek_lba();
                let mut data = [0u8; 0x930];
                let _ = self.read_sector_raw(lba, &mut data);
            ...
            CdromDriveMode::Read => {
                ...
                let lba = self.get_seek_lba();
                let mut full_sector = [0u8; 0x930];
                let read_ok = self.read_sector_raw(lba, &mut full_sector);

                let header = CdromHeader::from_slice(&full_sector[0xc..]);
                let subheader = CdromSubheader::from_slice(&full_sector[0x10..]);
                ...
                self.last_subq.ss = header.second.saturating_sub(2);
                ...
                if read_ok && (header.minute != sm || header.second != ss || header.sector != sf) {
                    log_warn!(
                        "CDROM",
                        "sector header mismatch: expected {}:{}:{} found {}:{}:{}",
                        sm, ss, sf, header.minute, header.second, header.sector
                    );
                }

                if read_ok && header.mode != 2 {
                    log_warn!("CDROM", "unsupported MODE{} sector", header.mode);
                }
                ...
                match mode {
                    CdromSectorMode::Adpcm => {
                        if subheader.bit_depth() != 4 {
                            log_warn!("CDROM", "unsupported ADPCM bit depth, skipping sector");
                        ...
                        data.copy_from_slice(&full_sector[0x18..0x18 + 0x914]);
                        ...
                        // 37800 Hz -> 44100 Hz resample: emit 7 output samples
                        // every 6 input samples per channel. Keep the modulo-6
                        // counter per-channel so stereo XA channels stay phase
                        // aligned. (Previously a shared counter meant channel 1
                        // started off-phase from channel 0 and video audio
                        // sounded "off".)
                        for channel in 0..channels {
                            for _ in 0..times {
                                for i in 0..self.adpcm_buffers[channel].len() {
                                    self.ringbuf[channel][i & 0x1f] = self.adpcm_buffers[channel][i];
                                    self.sixstep_per_channel[channel] += 1;

                                    if self.sixstep_per_channel[channel] == 6 {
                                        self.sixstep_per_channel[channel] = 0;
                    ...
                    CdromSectorMode::Data => {
                        self.sector[..0x930].copy_from_slice(&full_sector);

                        // Note: drive_interrupt_pending can legitimately be
                        // true here during CD streaming; the next branch
                        // queues the stat for later, no warning needed.
    ...
    fn execute_command(&mut self, command: u8) {
        if command >= 0x20 {
            log_warn!("CDC", "invalid command {:#04x}, ignoring", command);
            self.push_stat();
            self.controller_interrupt_flags = 0x5;
            return;
        }
        ...
        match command {
            // CdlSync — Reads the current cdrom status. Some BIOS regions
            // (particularly PAL boot) issue this with zero parameters.
            0x00 => {
                self.push_stat();
            }
            0x01 => {
            ...
            _ => {
                log_warn!("CDROM", "unknown command {:#04x}, replying with stat", command);
                self.push_stat();
                interrupt = 0x5;
            }
        };
    ...
    fn execute_test_command(&mut self) {
        ...
        match command {
            ...
            _ => {
                log_warn!("CDROM", "unknown test command {:#04x}, ignoring", command);
                self.push_stat();
            }
        }
    }
    ...
    fn get_seek_location(&self) -> u64 {
        self.get_seek_lba() * BYTES_PER_SECTOR
    }

    fn get_seek_lba(&self) -> u64 {
    ...
    fn read_data(&mut self) -> u8 {
        ...
        if self.data_buffer_empty() {
            log_warn!("CDROM", "reading from empty data buffer");
