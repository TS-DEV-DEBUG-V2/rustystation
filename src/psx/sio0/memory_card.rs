        ...
        match self.state {
            ...
            1 => {
                ...
                match command {
                    ...
                    0x53 => self.state = 2,
                    0x57 => self.state = 21,
                    // PocketStation probes (0x58..0x5d) and other vendor
                    // commands: a real card just doesn't ACK. Don't ACK and
                    // don't spam the log — games poll these every few frames.
                    _ => {
                        self.state = 0;
                        self.ack = false;
                    },
            ...
            29 => {
                if self.checksum_match {
                    ...
                    log_warn!("MCD", "checksum mismatch: {:#x}", self.checksum);
                    reply = 0x4e;
                }
