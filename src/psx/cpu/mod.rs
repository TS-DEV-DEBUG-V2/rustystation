    ...
    fn enter_exception(&mut self, exception: Exception) {
        ...
        if (exception != Exception::Interrupt)
           && (exception != Exception::Syscall) {
            // For data-fault style exceptions the bad virtual address is the
            // most useful clue. AddrLoad/AddrStore and DBusError are the ones
            // we care about; for the rest, bad_vaddr is stale but the EPC +
            // faulting instruction word still tell us where things went off
            // the rails.
