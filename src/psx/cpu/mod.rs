...
pub struct R3000A {
    ...
    // De-spam for unexpected_exception log: same exception at the same PC
    // with the same bad_vaddr only prints once until it changes.
    #[serde(skip)]
    last_exception_log: (u8, u32, u32, u32),
...
impl R3000A {
    ...
    fn op_cop0(&mut self, i: Instruction) {
        match i.rs() {
            ...
            0x10 => self.op_rfe(),
            _ => {
                crate::log_warn!("CPU", "unrecognised COP0 instruction {:#010x}", i.0);
    ...
    fn op_cop2(&mut self, i: Instruction) {
        match i.rs() & 0x10 {
            0x00 => match i.rs() & 0x0f {
                ...
                _ => {
                    crate::log_warn!("CPU", "unrecognised COP2 instruction {:#010x}", i.0);
                    self.enter_exception(Exception::Reserved);
                }
            },
    ...
    fn reg(&self, index: usize) -> u32 {
        // Safety/perf: regs is a fixed-size array. Index always comes from a
        // 5-bit field via Instruction::rs/rt/rd, so it is bounded to 0..32.
        // Bounds checks would still fire because the compiler can't see the
        // bit-mask invariant across calls; an inline accessor lets LLVM hoist
        // checks when possible without us reaching for `unsafe`.
        self.regs[index]
    }
    ...
    fn enter_exception(&mut self, exception: Exception) {
        ...
        if (exception != Exception::Interrupt)
           && (exception != Exception::Syscall) {
            // Dedupe consecutive identical faults so games that spin in their
            // own exception handler don't flood the log.
            let bad = self.cop0.bad_vaddr();
            let sig = (exception as u8, epc, self.current_instruction, bad);
            if sig != self.last_exception_log {
                let in_bd = if bd { " (delay slot)" } else { "" };
                crate::log_warn!(
                    "CPU",
                    "unexpected exception: {:?}{} at PC={:#010x} ins={:#010x} bad_vaddr={:#010x}",
                    exception, in_bd, epc, self.current_instruction, bad
                );
                self.last_exception_log = sig;
            }
           }
    ...
    pub fn translate_address(virtual_address: u32) -> u32 {
        // KSEG0/KSEG1/KUSEG mirror mask. Table-indexed by virtual_address >> 29.
        // Avoids a match per load/store. Equivalent to the original logic:
        //   0b000..=0b011 (KUSEG)        -> unchanged
        //   0b100        (KSEG0, cached) -> & 0x7fff_ffff
        //   0b101        (KSEG1, uncached) -> & 0x1fff_ffff
        //   0b110..=0b111 (KSEG2)         -> unchanged
        const REGION_MASK: [u32; 8] = [
            0xffff_ffff, 0xffff_ffff, 0xffff_ffff, 0xffff_ffff,
            0x7fff_ffff, 0x1fff_ffff, 0xffff_ffff, 0xffff_ffff,
        ];
        virtual_address & REGION_MASK[(virtual_address >> 29) as usize]
    }
    ...
    fn load(&mut self,
        ...
        let (val, err) = match physical_address {
        ...
        // Stash the faulting virtual address so DBusError logging at the call
        // site shows the real target, not stale data from a prior fault.
        if err {
            self.cop0.set_bad_vaddr(address);
        }
    ...
    fn store(&mut self,
        ...
        let err = match physical_address {
