...
impl Bus {
    pub fn new(game_filepath: &str) -> Bus {
        // Start from a mutable copy of the embedded BIOS so we can patch in
        // TTY-output and (optionally) fast-boot trampolines without touching
        // the read-only .rodata image.
        let mut bios: Box<[u8]> = bios::BIOS[..].to_vec().into_boxed_slice();
