...
impl Bus {
    pub fn new(game_filepath: &str) -> Bus {
        Self::new_with_bios(game_filepath, None)
    }

    pub fn new_with_bios(game_filepath: &str, custom_bios: Option<&[u8]>) -> Bus {
        // Start from a mutable copy of the BIOS (custom-provided or embedded)
        // so we can patch in TTY-output and (optionally) fast-boot
        // trampolines without touching the read-only image.
        let source: &[u8] = match custom_bios {
            Some(b) if b.len() == bios::BIOS_SIZE => b,
            Some(b) => {
                crate::log_warn!(
                    "BIOS",
                    "custom BIOS is {} bytes, expected {}; falling back to embedded",
                    b.len(), bios::BIOS_SIZE,
                );
                &bios::BIOS[..]
            }
            None => &bios::BIOS[..],
        };
        let mut bios: Box<[u8]> = source.to_vec().into_boxed_slice();
    ...
    pub fn load(&mut self, tk: &mut Timekeeper, width: BusWidth, address: u32) -> (u32, bool) {
        ...
        let value = match address {
            ...
            // SIO1 (serial expansion port). Not emulated; many games still
            // poke its registers during init. Return 0 on read.
            0x1f80_1050..=0x1f80_105f => 0,
    ...
    pub fn store(&mut self, tk: &mut Timekeeper, width: BusWidth, address: u32, value: u32) -> bool {
        ...
        match address {
            ...
            // SIO1 (serial expansion port). Not emulated; accept writes and
            // discard. Games hammer this during init regardless of use.
